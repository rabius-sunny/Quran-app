        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div className="space-y-0.5 flex-1 min-w-0">
              ...
              >
                <span className="truncate">{bookmark.surahName || `Surah ${surahNo}`}</span>
                <Badge
                  variant="outline"
...
function BookmarksPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {bookmarks.length === 0 ? (
        /* Ornate Empty State */
        <Card className="ornate-card animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          <CardContent className="p-12 md:p-16 text-center space-y-6 relative">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="space-y-3">
                <h3 className="text-2xl font-bold text-foreground">No bookmarks yet</h3>
                <p className="text-muted-foreground max-w-md mx-auto leading-relaxed">
                  Start bookmarking verses you want to revisit. Click the bookmark icon on any verse to save it here for easy access.
                </p>
              </div>

              <div className="gold-separator max-w-xs mx-auto" />

              <p className="arabic-text text-xl text-muted-foreground/50">
        ...
        <div className="space-y-4">
          {sortedBookmarks.map((bookmark, index) => (
            <BookmarkItem key={bookmark.verseKey} bookmark={bookmark} index={index} />
          ))}
        </div>
      )}
    </div>
