...
function SurahsPage() {
  ...
  const [revelationFilter, setRevelationFilter] = useState<'All' | 'Mecca' | 'Madina'>('All')
  ...
  const filteredSurahs = useMemo(() => {
    ...
    return surahs
      ...
      .filter((item) => {
        // Search filter
        const matchesSearch = 
          item.surah.surahName.toLowerCase().includes(searchQuery.toLowerCase()) ||
          item.surah.surahNameArabic.includes(searchQuery) ||
          item.surah.surahNameTranslation.toLowerCase().includes(searchQuery.toLowerCase()) ||
          item.number.toString().includes(searchQuery)

        // Revelation place filter
        const matchesRevelation = 
          revelationFilter === 'All' || item.surah.revelationPlace === revelationFilter
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="space-y-6 relative overflow-hidden rounded-3xl p-8 md:p-12 glass-card animate-fade-in-up">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-lg text-muted-foreground mt-2 max-w-2xl">
                Browse and search through all {stats.total} chapters of the Holy Quran with advanced filtering options.
              </p>
            </div>
            
            {/* Stats */}
            <div className="flex flex-wrap gap-3">
              <Badge variant="outline" className="text-sm py-2 px-4 glass-card bg-background/50">
                <span className="font-bold text-primary mr-1">{stats.total}</span>
                Total Surahs
              </Badge>
              <Badge variant="outline" className="text-sm py-2 px-4 glass-card bg-background/50">
                <MapPin className="w-3.5 h-3.5 mr-1 text-emerald-600 dark:text-emerald-400" />
                <span className="font-bold text-emerald-600 dark:text-emerald-400 mr-1">{stats.meccan}</span>
                Meccan
              </Badge>
              <Badge variant="outline" className="text-sm py-2 px-4 glass-card bg-background/50">
                <MapPin className="w-3.5 h-3.5 mr-1 text-teal-600 dark:text-teal-400" />
                <span className="font-bold text-teal-600 dark:text-teal-400 mr-1">{stats.medinan}</span>
                Medinan
              </Badge>
            </div>
          </div>
      ...
      {/* Search and Filters */}
      <div className="sticky top-20 z-30 bg-background/80 backdrop-blur-xl p-4 -mx-4 md:mx-0 rounded-2xl border border-border/50 shadow-sm animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="relative flex-1">
            ...
            <Input
              type="text"
              placeholder="Search by name (English/Arabic), number, or translation..."
              value={searchQuery}
          ...
          <div className="flex gap-2 p-1 bg-muted/30 rounded-lg border border-border/50">
            ...
            <Button
              ...
              size="sm"
              className={revelationFilter === 'Mecca' ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm' : ''}
            >
              Mecca
            </Button>
            <Button
              ...
              size="sm"
              className={revelationFilter === 'Madina' ? 'bg-teal-600 hover:bg-teal-700 text-white shadow-sm' : ''}
            >
              Madina
            </Button>
          </div>
        ...
        <Card className="glass-card border-dashed">
          <CardContent className="p-16 text-center">
            ...
            <p className="text-muted-foreground max-w-sm mx-auto">
              We couldn't find any surahs matching "{searchQuery}". Try adjusting your search or filters.
            </p>
            <Button 
              variant="outline" 
              className="mt-6"
