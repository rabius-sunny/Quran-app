...
// Debounced save to localStorage (don't save isPlaying and currentVerse)
const debouncedSave = debounce((state: AudioState) => {
  const { isPlaying, currentVerse, ...persistedState } = state
...
// Actions
export function play(surahNo: number, ayahNo: number) {
